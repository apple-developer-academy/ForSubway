...
public class PopDatePicker : NSObject, UIPopoverPresentationControllerDelegate, DataPickerViewControllerDelegate {
    ...
    public func pick(inViewController : UIViewController, initDate : NSDate?, modePicker:UIDatePickerMode, dateMin:NSString, dateMax:NSString, dataChanged : PopDatePickerCallback) {
        Type.modePicker = modePicker
        let dateFormatter = NSDateFormatter()
        dateFormatter.dateFormat = "dd-MM-yyyy HH:mm"
        
        Type.dateMin = dateFormatter.dateFromString(String(dateMin))!
        Type.dateMax = dateFormatter.dateFromString(String(dateMax))!
        if presented {
            return  // we are busy
        }
